...
class GafferExtension {
    ...
    private addWebapp(Closure closure, WebappAssemble assemble) {
        ...
        project.tasks.register("war-"+assemble.name,War){
            dependsOn:webappAssembleTaskName
    ...
    private addContainer(ContainerAssemble containerAssemble,Closure closure){
        ...
        //create task wit
        ContainerAssembleTask containerTask = project.tasks.register(taskName,ContainerAssembleTask){
            ...
            group:"container assemble"
            dependsOn containerDependencies
            setAssemble(containerAssemble)
            setContainerAssemble(containerAssemble)
            targetDirectory=assembleTargetDir
            deploymentReport = report
            applicationAssembles.addAll(applicationAssembles)
            profileApplicationAssembles.addAll(profileAssembles)
        }
    }
