...
----
plugins {
  id "de.ethinking.gaffer" version "0.7.2"
}
----
...
The Gradle Gaffer plugin will generate a set of dynamic tasks from the declared applications, profiles and containers inside the gaffer extenstion. You can use the tasks and its output in other task to deploy the assembled containers by ssh or into a Docker container.
...
== Gaffer Report
The Gaffer Report is a small AngularJS App which provide informations about the assembled files. You must activate the report by setup the *reportTarget* property inside the container to a desired location.
